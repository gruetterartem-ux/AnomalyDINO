...
DEFAULT_METRICS_CSV = Path(
    r"C:\ai\AnomalyDINO\results_CUSTOM\dinov3_vitb16_688\8-shot_preprocess=force_no_mask_no_rotation_bestsearch8_fast20greedy_maxanomap_res688_evaltrain_20260413\roi_normalmap_bbox_metrics_local_ring_ignore_black\roi_normalmap_bbox_metrics.csv"
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train/evaluate a Random Forest on labeled ROI bbox metric features with StratifiedGroupKFold."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="label",
        help="Column containing the class labels, e.g. label or component_label.",
    )
    parser.add_argument(
        "--feature-columns",
        type=str,
        nargs="*",
        default=[
            "peak_inclination_p99_deg",
            "relative_peak_inclination_p99_deg",
            "gradient_max",
            "divergence_min",
            "divergence_max",
        ],
        help="Metric columns used as Random Forest input features.",
    )
...
def default_output_dir(metrics_csv: Path, explicit_output_dir: Path | None) -> Path:
    if explicit_output_dir is not None:
        return explicit_output_dir.resolve()
    return (metrics_csv.parent / "random_forest_groupcv_results").resolve()
...
def main():
    ...
    output_dir = default_output_dir(metrics_csv, args.output_dir)
    ...
    for fold_index, (train_idx, val_idx) in enumerate(splitter.split(X, y, groups), start=1):
        ...
        for feature_name, importance in zip(feature_columns, model.feature_importances_.tolist()):
    ...
    write_json(
        {
            ...
            "folds": fold_rows,
            "mean_feature_importances": mean_importance_rows,
            "fold_metrics_file": str(fold_metrics_file),
            "oof_predictions_file": str(oof_predictions_file),
            "feature_importances_by_fold_file": str(feature_importances_file),
