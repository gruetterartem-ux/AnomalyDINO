...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "the image threshold. Aggregate per layer with elementwise max and std, then concatenate."
        )
    )
...
def default_output_dir(experiment_dir: Path, explicit_output_dir: Path | None) -> Path:
    ...
    return (
        experiment_dir
        / "roi_overthreshold_overlap_multilayer_l1to12_maxstd_features_labeled"
    ).resolve()


def select_overlap_threshold_patches(
    row: pd.Series,
    meta: dict,
    ...
    for patch_row in range(row_min, row_max):
        for patch_col in range(col_min, col_max):
            if not patch_overlaps_bbox(patch_row, patch_col, meta, x_min, y_min, x_max, y_max):
                continue
            score = float(anomaly_grid[patch_row, patch_col])
            overlap_candidates.append((score, patch_row, patch_col))

    if not overlap_candidates:
        raise ValueError(f"No overlap candidates found for ROI {row.get('roi_uid', '<unknown>')}")

    overlap_candidates.sort(key=lambda item: item[0], reverse=True)
    selected = [(patch_row, patch_col) for score, patch_row, patch_col in overlap_candidates if score > float(image_threshold)]
    if selected:
        selection_mode = "overlap_over_threshold"
    else:
        top_score, top_row, top_col = overlap_candidates[0]
        selected = [(top_row, top_col)]
        selection_mode = "overlap_threshold_fallback_max"
    return selected, selection_mode, len(overlap_candidates)


def aggregate_maxstd_per_layer(features_layers: np.ndarray, selected_patches: list[tuple[int, int]], grid_shape: tuple[int, int]) -> np.ndarray:
    ...
    std_pool = selected_layers.std(axis=0, ddof=0)
    per_layer = np.concatenate([max_pool, std_pool], axis=1).astype(np.float32)
...
def main() -> None:
    ...
    for feature_index, row in enumerate(labeled_rois.itertuples(index=False), start=0):
        ...
        if layer_indices_ref is None:
            ...
            feature_dim_ref = int(features_layers.shape[1] * features_layers.shape[2] * 2)
        else:
        ...
        combined_feature = aggregate_maxstd_per_layer(features_layers, selected_patches, grid_shape)
        ...
        table_rows.append(
            {
                ...
                "feature_mode": f"multilayer_maxstd_overthreshold_overlap_l{layer_indices[0]}to{layer_indices[-1]}",
    ...
    summary = {
        ...
        "layer_dim": int((feature_dim_ref or 0) // max(2 * len(layer_indices_ref or [1]), 1)),
